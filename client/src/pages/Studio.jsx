...
function Studio() {
  ...
  useEffect(() => {
    if (isNewComposition) {
      ...
      const fetchComposition = async () => {
        try {
          const response = awaitaxios.get(import.meta.env.VITE_API_URL + `api/compositions/${id}`);
  ...
  const handleSaveComposition = async () => {
    try {
      ...
      if (isNewComposition) {
        response = await axios.post(import.meta.env.VITE_API_URL + 'api/compositions', composition);
        // Redirect to the new composition's edit page
        navigate(`/studio/${response.data._id}`, { replace: true });
      } else {
        response = await axios.put(import.meta.env.VITE_API_URL +  `api/compositions/${id}`, composition);
      }
